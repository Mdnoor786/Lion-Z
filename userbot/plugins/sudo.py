...
@lionub.lion_cmd(
    ...
    info={
        "header": "To enable or disable sudo of your Lion-Z.",
...
async def chat_blacklist(event):
    "To enable or disable sudo of your LionZ."
...
async def _(event):  # sourcery no-metrics
    ...
    if input_str[0] == "-all":
        ...
        for plugin in input_str:
            if plugin not in PLG_INFO:
                errors += f"`{cmd}` __There is no such plugin in your LionZ__.\n"
            else:
        ...
        for cmd in input_str:
            if cmd not in CMD_INFO:
                errors += f"`{cmd}` __There is no such command in your LionZ__.\n"
    ...
    result = f"__Successfully enabled __ `{len(loadcmds)}` __ for LionZ sudo.__\n"
    output = (
...
async def _(event):  # sourcery no-metrics
    ...
    result = f"__Successfully disabled __ `{count}` __ for LionZ sudo.__\n"
    output = (
        result + "**Bot is reloading to apply the changes. Please wait for a minute**\n"
    )
    if errors != "":
