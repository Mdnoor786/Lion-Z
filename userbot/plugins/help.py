...
async def cmdlist():
    outstr = "**Total list of Commands in your Lion-Z are :**\n\n"
    category = ["admin", "bot", "fun", "misc", "tools", "utils", "extra"]
    for lion in category:
        plugins = GRP_INFO[lion]
        outstr += f"**{hemojis[lion]} {lion.title()} ** - {len(plugins)}\n\n"
...
@lionub.lion_cmd(
    ...
    info={
        "header": "To get guide for Lion-Z.",
...
async def _(event):
    ...
    if found := [i for i in sorted(list(CMD_INFO)) if cmd in i]:
        ...
        out = f"I can't find any such command `{cmd}` in LionZ"
