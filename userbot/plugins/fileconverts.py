...
async def pic_gifcmd(event):  # sourcery no-metrics
    ...
    crazy = await event.client.send_file(
        event.chat_id,
        media,
        reply_to=reply,
        video_note=True,
    ...
    if output is None:
        ...
        for i in [final, "Output.gif", imag[1]]:
            if os.path.exists(i):
                os.remove(i)
        return
    crazy = await event.client.send_file(event.chat_id, output, reply_to=reply)
    await _lionutils.unsavegif(event, crazy)
    await lionevent.delete()
    for i in [final, "Output.gif", imag[1]]:
        if os.path.exists(i):
...
async def _(event):
    ...
    crazy = await event.client.send_file(event.chat_id, result, reply_to=reply)
    await _lionutils.unsavegif(event, crazy)
    await lionevent.delete()
