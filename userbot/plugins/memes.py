...
# LION-Z module for having some fun with people.
import asyncio
import random
import re
...
@lionub.lion_cmd(
    ...
    info={
        ...
        "examples": [
            "{tr}squirrelsay Lion-Z",
            "{tr}milksay Lion-Z",
            "{tr}ghostbustersghostbusterssay Lion-Z",
        ],
    },
)
...
async def decide(event):
    ...
    await event.delete()
    crazy = await event.client.send_message(
        event.chat_id, str(r["answer"]).upper(), reply_to=message_id, file=r["image"]
    )
    await _lionutils.unsavegif(event, crazy)
    ...
    if event.reply_to_msg_id:
        ...
        if idd == 1837687523:
            await lionevent.edit(
                "`Wait a second, This is my master!`\n**How dare you threaten to ban my master nigger!**\n\n__Your account has been hacked! Pay 69$ to my master__ [π.$](tg://user?id=1837687523) __to release your account__😏"
            )
        else:
            jnl = (
                "`Warning!! `"
