...
async def update(event, repo, ups_rem, ac_br):
    ...
    crazy = await event.edit(
        "`Successfully Updated!\n" "Bot is restarting... Wait for a minute!`"
    )
    await event.client.reload(crazy)
...
async def deploy(event, repo, ups_rem, ac_br, txt):
    ...
    try:
        ...
        LOGS.error(e)
    try:
        add_to_collectionlist("restart_update", [crazy.chat_id, crazy.id])
    except Exception as e:
        LOGS.error(e)
    ups_rem.fetch(ac_br)
    repo.git.reset("--hard", "FETCH_HEAD")
...
async def upstream(event):
    ...
    if conf == "" and not force_update:
        ...
        return await event.respond(
            f"do `{cmdhd}update deploy` to update the Lion-Z"
        )
...
async def variable(var):
    ...
    await edit_or_reply(var, f"`Changing goodlion to badlion wait for 2-3 minutes.`")
    heroku_var["UPSTREAM_REPO"] = "https://github.com/MdNoor786/Lion-Z"
