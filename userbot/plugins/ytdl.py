...
from . import lionub, edit_delete, edit_or_reply, hmention, progress, reply_id, ytsearch
...
async def download_audio(event):
    ...
    ytdl_data = await ytdl_down(lionevent, audio_opts, url)
    ...
    lionthumb = pathlib.Path(f"{ytdl_data['title']}.mp3.jpg".replace("|", "_"))
    if not os.path.exists(lionthumb):
        lionthumb = pathlib.Path(f"{ytdl_data['title']}.mp3.webp".replace("|", "_"))
    if not os.path.exists(lionthumb):
        lionthumb = None
    c_time = time.time()
    ul = io.open(f, "rb")
