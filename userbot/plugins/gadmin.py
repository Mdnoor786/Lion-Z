...
async def liongban(event):  # sourcery no-metrics
    ...
    mad = await admin_groups(event.client)
    count = 0
    crazy = len(mad)
    if crazy == 0:
        return await edit_delete(lionz, "`you are not admin of atleast one group` ")
    await lionz.edit(
        f"`initiating gban of the `[user](tg://user?id={user.id}) `in {len(mad)} groups`"
    )
    for i in range(crazy):
        try:
            await event.client(EditBannedRequest(mad[i], user.id, BANNED_RIGHTS))
            await asyncio.sleep(0.5)
            count += 1
        except BadRequestError:
            achat = await event.client.get_entity(mad[i])
...
async def liongban(event):
    ...
    await lionz.edit(
        f"initiating ungban of the [user](tg://user?id={user.id}) in `{len(mad)}` groups"
    )
    for i in range(crazy):
        try:
            await event.client(EditBannedRequest(mad[i], user.id, UNBAN_RIGHTS))
            await asyncio.sleep(0.5)
            count += 1
        except BadRequestError:
            achat = await event.client.get_entity(mad[i])
...
async def liongkick(event):  # sourcery no-metrics
    ...
    if crazy == 0:
        return await edit_delete(lionz, "`you are not admin of atleast one group` ")
    await lionz.edit(
        f"`initiating gkick of the `[user](tg://user?id={user.id}) `in {len(mad)} groups`"
    )
    for i in range(crazy):
        try:
            await event.client.kick_participant(mad[i], user.id)
            await asyncio.sleep(0.5)
            count += 1
        except BadRequestError:
            achat = await event.client.get_entity(mad[i])
