...
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("**Lion-Z"):
            buttons = [
                (
                    Button.inline("Stats", data="stats"),
                    Button.url("Repo", "https://github.com/MdNoor786/Lion-Z"),
                )
            ]
            ...
            try:
                ...
                try:
                    ...
                    if u.username:
                        ...
                        meow = f"[{u.first_name}](tg://user?id={u.id})"
                except ValueError:
                    # ValueError: Could not find the input entity
                    meow = f"[user](tg://user?id={u})"
            except ValueError:
                # if u is username
                try:
            ...
            result = builder.article(
                ...
                text=f"🔒 A whisper message to {meow}, Only he/she can open it.",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            if jsondata:
            ...
            result = builder.article(
                title="© Lion-Z Help",
                description="Help menu for Lion-Z",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
        ...
        buttons = [
            (
                Button.url("Source code", "https://github.com/MdNoor786/Lion-Z"),
                Button.url(
                    "Deploy",
                    "https://dashboard.heroku.com/new?button-url=https%3A%2F%2Fgithub.com%2FMdNoor786%2Flionpack&template=https%3A%2F%2Fgithub.com%2FMdNoor786%2Flionpack",
                ),
            )
        ]
        markup = event.client.build_reply_markup(buttons)
        ...
        result = types.InputBotInlineResult(
            ...
            url="https://github.com/MdNoor786/Lion-Z",
            thumb=photo,
            content=photo,
            send_message=types.InputBotInlineMessageMediaAuto(
                reply_markup=markup, message=text, entities=msg_entities
